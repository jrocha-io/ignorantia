...
LEIA `references/artifact-vocabulary-policy.xml` ANTES de gravar QUALQUER prosa em disco que ficará no pacote Zenodo (manuscript.tex, protocol.md, README.md, compliance-checklist.md, ai-declaration.md, venue-suggestions.md, avaliacao_*.md, gap_report.md, citations_to_obtain_*.md) — Decisão 41, Fix 19 do RS-42 remediation. A política mapeia 6 classes de label operacional da skill (Tier N, FALLBACK_MD, KEY → PROXY, Decisão N, DD-N, valores enum de `review_purpose`/`review_type`, meta-projeto `projeto subjacente`/`outras N revisões`/`função declaratória/instrumental`, SemVer rhetoric, brand) para suas traduções obrigatórias em prosa científica. Enum-values de `review_purpose`/`review_type` ficam **apenas no metadado JSON** do pacote — nunca em corpo de prosa. Em prosa, traduzir com a tabela.
EXECUTE antes do empacotamento Zenodo `python3 scripts/check_decision_19_vocabulary.py --all <output_dir> --gate-sidecar --quiet` — gate de vocabulário deposit-wide (Decisão 41, Fix 19 do RS-42 remediation, expandido em v2.23.3). Varre TODOS os artefatos `.md`/`.tex`/`.html` do depósito (não só manuscript.tex como na v2.23.1) procurando 12 classes de label skill-internal: SemVer rhetoric, `Decisão N`, `DD-N`, `Categoria A/B`, `Tier N`, `FALLBACK_MD`, `KEY → PROXY`, valores enum de `review_purpose`, `projeto subjacente`, `outras N revisões`, `função declaratória/instrumental`, brand `ignorantia`. Exit code 2 em qualquer violação em qualquer arquivo; emite sidecar `vocabulary_gate.json` em `<output_dir>` com `passed/total_violations/files_with_violations/violations_by_label`. Encadear via `&&` antes do ZIP. RS-42 v1.0.0 falharia com 28 violações; RS-42 Mark 4 falharia com 92 violações em 4 arquivos. `--no-gate` existe apenas para triagem em sessões de debug; nunca em fluxo de produção.
...
DECLARE no metadado JSON do pacote (`reproducibility_manifest.json` ou equivalente) o valor de `review_type`. Valores válidos v2.1 (ver `references/modes/MODES_OVERVIEW.xml` e `VALID_REVIEW_TYPES_V21` em `scripts/assessor/eliminators.py`): camada primária = scoping_review | rapid_review | mapping_study | integrative_review | realist_review; camada secundária = software_paper | position_paper | theoretical_essay | technical_report | white_paper | policy_brief; camada terciária = systematic_review_with_2_reviewers — Decisões 1, 10 v2.1. **Os valores literais de enum NÃO entram em prosa** — em §03/§04 do manuscrito, traduza para vocabulário científico ("Esta scoping review segue PRISMA-ScR…"; "rapid review conduzida em 30 dias…"; etc.) seguindo `references/artifact-vocabulary-policy.xml`. A literal `scoping_review` no corpo do paper é violação da Decisão 41 e bloqueia o empacotamento.
DECLARE no metadado JSON do pacote o valor de `review_purpose` (design_foundational | design_validation | design_correction | independent_inquiry) — Decisão 8 v2.0. **Os valores literais de enum NÃO entram em prosa** — em §00/§01 do manuscrito, traduza ("uma revisão metodológica que fundamenta as escolhas de design subsequentes deste autor no projeto X" para `design_foundational`; etc.). A literal `design_foundational` no corpo do paper é violação da Decisão 41 e bloqueia o empacotamento.
...
- `references/artifact-vocabulary-policy.xml` — **vinculante (Decisão 41, Fix 19)** — tabela de tradução skill-internal-label → prosa científica que Claude lê ANTES de gravar qualquer prosa em disco que vá pro pacote Zenodo. Pareada com `scripts/check_decision_19_vocabulary.py --all`.
